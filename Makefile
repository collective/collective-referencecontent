...
PROJECT_NAME=collective-referencecontent
STACK_NAME=collective-referencecontent-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://collective-referencecontent.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t collective/collective-referencecontent-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t collective/collective-referencecontent-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name collective-referencecontent-frontend-acceptance --link collective-referencecontent-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/collective-referencecontent-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name collective-referencecontent-backend-acceptance -d collective/collective-referencecontent-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop collective-referencecontent-frontend-acceptance
	@docker stop collective-referencecontent-backend-acceptance
